...
train_data = list(map(lambda mat: list(chain(mat)), train_data))
...
model = ColoringNN(in_size=IN_SIZE * IN_SIZE, out_size=OUT_SIZE, hidden_size=HIDDEN_SIZE, num_layers=4, num_nodes=NUM_NODES)
optimizer = torch.optim.Adam(params=model.parameters(), lr=LEARNING_RATE, weight_decay=0.011)
...
torch.save(model, 'model_original_clique.pth')